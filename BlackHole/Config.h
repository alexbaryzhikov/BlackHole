// Display
#define VIEWPORT_WIDTH              1600
#define VIEWPORT_HEIGHT             900
#define MAIN_TEXTURE_WIDTH          3200
#define MAIN_TEXTURE_HEIGHT         1800
#define HDR_ENABLED                 true

// Camera
#define CAMERA_HORIZONTAL_FOV       1.04719758f
#define CAMERA_EXPOSURE_DELTA       0.03125f
#define CAMERA_ORBIT_MIN            10.0f
#define CAMERA_ORBIT_MAX            1000.0f

// Mouse
#define MOUSE_SENSITIVITY           1.0f
#define MOUSE_ZOOM_SENSITIVITY      1.0f

// Accretion disk
#define DISK_INNER_RADIUS           3.2f
#define DISK_OUTER_RADIUS           15.0f
#define PARTICLE_COUNT              500000
#define DENSITY_MAP_RESOLUTION      4096
